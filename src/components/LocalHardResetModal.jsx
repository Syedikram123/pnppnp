...
import { requestLocalHardReset } from '../services/adminService';
import { AlertTriangle, X, Loader2, CheckCircle } from 'lucide-react';

export default function LocalHardResetModal({ onClose, onSuccess }) {
  ...
  const handleReset = async (e) => {
    ...
    setLoading(true);
    try {
      const result = await requestLocalHardReset(password);
      ...
      <div className="bg-navy-900 border border-rose-500/50 rounded-2xl w-full max-w-md shadow-2xl p-6 space-y-5">
        ...
        <div className="flex items-center justify-between border-b border-navy-800 pb-3">
          <div className="flex items-center gap-2 text-rose-400 font-bold text-base">
            ...
            <span>Hard Reset Local Database</span>
          </div>
        ...
        {step === 1 ? (
          <div className="space-y-4">
            <div className="bg-rose-500/10 p-4 rounded-xl border border-rose-500/30 text-rose-200 text-xs leading-relaxed space-y-2">
              ...
              <p>
                This will permanently delete all locally stored products, transactions and stock data.
              </p>
